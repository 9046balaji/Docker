...
There are three important steps in the lifecycle:

1. **`docker build`** $\rightarrow$ Builds Docker images from a Dockerfile.
2. **`docker run`** $\rightarrow$ Runs containers from Docker images.
3. **`docker push`** $\rightarrow$ Pushes the container image to public or private registries to share the Docker image.

---

### Dockerfile

It is a set of instructions used to build an image.
...
* When you use the `docker push` command, your image is pushed and saved to your configured registry.

### Docker Bind Mounts and Volumes

### (1) Bind Mounts

* It allows you to bind any folder or file on your host computer directly into a container.
* **How:** You specify an existing host path and a destination container path so the container can see and modify those host files instantly.
...
### Container $\leftrightarrow$ Host: Bridge Networking
 ...
 *CRITICAL NOTE: Without the 'docker0' bridge acting as a translator, 
  a network error occurs because the container app cannot directly 
  reach out to the host's physical network adapter.
...
Here is the structured translation and breakdown of your notes on Docker network drivers and container-to-container isolation boundaries.
...
* In this mode, Docker completely removes network isolation between the container and the host machine.
* The container binds directly to the host's network interfaces (`veth`).
...
* This driver is used when you need to establish a secure cluster **across multiple physical or virtual hosts**.
* It acts as the backbone layer for container orchestration engines like **Kubernetes (K8s)** and **Docker Swarm**.
...
* **The Flaw:** By default, every container hooked into the default `docker0` bridge can see, discover, and `ping` every other container on that same bridge using their internal `veth` paths.
* If a hacker compromises your exposed **Login Container (C1)**, they can freely traverse the internal bridge network to sniff or attack your private **Payments Container (C2)**.
...
* **How it works:** By creating separate custom bridge networks (e.g., `login-net` and `payment-net`), containers on different networks cannot communicate with each other by default.
* This provides complete, zero-trust network isolation between different app tiers right inside the host's virtual networking engine itself.
...
* **Sequential IP Assignment:** Docker running as a local network coordinator automatically allocates unique, isolated IP addresses within the same subnet pool (typically starting from `172.17.0.2` onwards) to each sequential container container instance (`C1` $\rightarrow$ `C2` $\rightarrow$ `C3` $\rightarrow$ `C4`).
